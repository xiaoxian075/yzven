...
public final class StringUtils {
	...
	public static String repeat(String str,int repeat){
		int len = 0;
		if(str == null || (len = str.length()) == 0) return str;
		if(repeat <= 1)
			return repeat == 0 ? "" : str;
		//计算需要的数组长度
		long newSize = (long)len * (long)repeat;//转型
		int size = (int)newSize;//向下转型
		if(size != newSize){
			//超出Int的最大值
			throw new ArrayIndexOutOfBoundsException("Required array size too large: " + newSize);
		}
		char[] array = new char[size];
		str.getChars(0, len, array, 0);//将原始字符串转换成char数组到array中
	    int n;
	...
	public static boolean isAnyEmpty(CharSequence... css){
		int len = 0;
		if(css == null || (len = css.length) == 0) return true;
	...
	public static String replace(String str,String oldChar,String newChar,int replaceMax){
		...
		if(str == null || (sl = str.length()) == 0
				...
				|| newChar == null || replaceMax == 0) return str;
		//判断字符中是否包括了要替换的字符串
		int start = 0, end;
