    ...
    initComponent : function () {
        ...
        if((store = _self._store)){
            ...
            if(typeof first == 'string' && store[1] && typeof store[2] == 'string'){
                ...
                //异步加载
                _self.queryMode = _self.queryMode || 'remote';//远程加载
        ...
        if(!_self.listeners)
            _self.listeners ={};
        if(_self._linkTo){
            _self._autoLoad = false;
        }
        _self.oldChange = _self.listeners.change;
        _self.listeners.change = function(field,oldVal,newVal,opts){
            if(field.__link){
                if(field.__link.isHidden()){
                    field.__link.show();
                }
                var store = field.__link.getStore();
                store._extParam = field.__link._store[3] || undefined;
                store._extParamVal = field.getValue();
                store.load();
            }
            if(field.oldChange)
                field.oldChange(field,oldVal,newVal,opts);
        };
        ...
        _self.listeners.afterrender = function(thisCom,opts){
            if(thisCom._copyCombo){//combo组件数据复制
                ...
                if(thisCom._autoLoad) thisCom.getStore().load();
            }
            var linkTo = thisCom._linkTo;
            if(linkTo){
                var dom = Ext.getCmp(linkTo);
                if(dom){
                    dom.__link = thisCom;
                }
            }
